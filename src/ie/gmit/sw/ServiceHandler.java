 ...
 */
@WebServlet("/doProcess")   // 50MB. he maximum size allowed for a multipart/form-data request, in bytes.
public class ServiceHandler extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		System.out.println("doProcess doGetCalled");
		...
		if (taskNumber == null){
			...
			jobNumber++;
			//Add job to in-queue
			r = new InQueue(jobNumber,req.getParameter("word")); // new instance of the thread
			new Thread(r).start();	
		}else{
			RequestDispatcher dispatcher = req.getRequestDispatcher("/poll");
			//dispatcher.forward(req,resp);
			System.out.println("Forwarding to poll");
			if (r == null) {
								// start the thread
				r = new InQueue(jobNumber,req.getParameter("word")); // new instance of the thread
				new Thread(r).start();	
			...
			try {
				...
				result = r.getResponse();							// get the response from our thread
				resp.sendRedirect("/poll?result="+result);					// call the get
		...
		out.print("<H3>Word to Search: " + title + "</H3>");
		...
		out.print("<script>");
		//out.print("var wait=setTimeout(\"document.frmRequestDetails.submit();\", 4000);"); //Refresh every 10 seconds
		out.print("</script>");
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("doProcess doPostCalled");
